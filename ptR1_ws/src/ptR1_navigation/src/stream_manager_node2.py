...
def handle_start_stream(req):
    ...
    if model is None:
        ...
        model = YOLO('/home/leoss/ptR1Project/yolo11n.onnx')
...
def ai_worker(frame):
    ...
    try:
        if model is None:
            return
        results = model.predict(source=frame, imgsz=320, conf=0.35, verbose=False)
        new_boxes = results[0].boxes.data.cpu().numpy()
        with ai_result_lock:
